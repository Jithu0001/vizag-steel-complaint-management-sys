...
exports.updateComplaintStatus = catchAsync(async (req, res, next) => {
  ...
  // 1. Fetch and populate complaint
  const complaint = await Complaint.findById(req.params.id)
    .populate('raisedBy')
    .populate('assignedTo');
  ...
  // 4. Create Audit Trail log entry
  await AuditLog.create({
  ...
  // 5. Emit WebSockets Real-time events
  const eventPayload = {
    complaintId: complaint._id,
    complaintNumber: complaint.complaintNumber,
    oldStatus,
  ...
  // 6. DB Native notification tracking
  ...
  // 7. BullMQ Background Notification Engine Queue Tasks
  try {
    const { emailQueue, smsQueue } = getQueues();

    if (emailQueue && complaint.raisedBy?.email) {
      await emailQueue.add('statusUpdate', {
        ...
        templateData: {
          complaintNumber: complaint.complaintNumber,
          newStatus: status,
          raisedByName: complaint.raisedBy.name || 'Employee',
        },
      });
      logger.info(`BullMQ: Email notification queued for ${complaint.complaintNumber}`);
    }

    if (smsQueue && complaint.raisedBy?.phone) {
      await smsQueue.add('statusUpdateSMS', {
        to: complaint.raisedBy.phone,
        message: `VSP-CMS: Your complaint #${complaint.complaintNumber} status changed to: ${status.toUpperCase()}`,
      });
      logger.info(`BullMQ: SMS notification queued for ${complaint.complaintNumber}`);
    }
  } catch (queueError) {
    logger.error('Failed to dispatch background jobs:', queueError.message);
  }

  // 8. CRITICAL FIX: Ensure exactly ONE single final response is sent here and nowhere else!
  ...
  // ─── 🔥 BULLMQ BACKGROUND NOTIFICATION DISPATCH ───────────────────────────
  try {
    const { emailQueue, smsQueue } = getQueues();

    // 1. Dispatch Email Job to the worker if the creator has an email address
    if (emailQueue && complaint.raisedBy?.email) {
      await emailQueue.add('statusUpdate', {
        to: complaint.raisedBy.email,
        templateName: 'statusUpdate',
    ...
    // 2. Dispatch SMS Job to the worker if the creator has a phone number
    if (smsQueue && complaint.raisedBy?.phone) {
      await smsQueue.add('statusUpdateSMS', {
        to: complaint.raisedBy.phone, // Ensure this number matches your verified trial number
        message: `VSP-CMS: Your complaint #${complaint.complaintNumber} status changed to: ${status.toUpperCase()}`,
      });
      logger.info(`BullMQ: SMS notification queued for ${complaint.complaintNumber}`);
    }
    ...
    logger.error('Failed to dispatch notification background jobs:', queueError.message);
  }
  // ───────────────────────────────────────────────────────────────────────────
  ...
  try {
    ...
    // 2. Optional: If a technician was just assigned, notify them too
    if (assignedTo && emailQueue && status === 'in_progress') {
      // Re-fetch clean info for assignee if newly attached
      const technician = await User.findById(assignedTo);
      if (technician?.email) {
        await emailQueue.add('complaintAssigned', {
          to: technician.email,
    ...
    // We catch errors to prevent notification queue connection faults from breaking the main request thread
    logger.error('Failed to dispatch notification background jobs:', queueError.message);
  }
  // ───────────────────────────────────────────────────────────────────────────
